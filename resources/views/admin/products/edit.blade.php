      ...
      <div class="row g-3">
          ...
          <div class="col-md-6">
              ...
              @if (!empty($product['image_url']))
                  ...
                  <img src="{{ $product['image_url'] }}" alt="Current image" class="rounded mt-1" style="width: 100px; height: 100px; object-fit: cover;">
              @endif
          </div>
          ...
          <div class="col-md-6">
              ...
              @if (!empty($product['model_url']))
                  <small class="text-muted d-block mt-1">Current: <a href="{{ route('stl.show', $id) }}" target="_blank">View existing model</a></small>
              @endif
          </div>

          {{-- ✅ 3D Preview --}}
          <div class="col-12 mt-4">
              <div class="bg-dark rounded" style="height:400px;">
                  <canvas id="previewCanvas" style="width:100%; height:100%; display:block;"></canvas>
              </div>
          </div>

          <div class="col-12 text-end mt-4">
              <button type="submit" class="btn btn-success px-4 shadow-sm">Update Product</button>
          </div>
      </div>
  </form>
</div>
...
function initPreview() {
    ...
    // Load existing model if present (via same-origin proxy)
    @if (!empty($product['model_url']))
        loadSTLFromUrl("{{ route('stl.show', $id) }}");
    @endif
}
